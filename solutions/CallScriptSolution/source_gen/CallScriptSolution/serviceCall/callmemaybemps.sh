#!/bin/sh
# ---------------------------------------------------------------------
# CallMeMaybeMPS startup script.
# ---------------------------------------------------------------------
# Generated by MPS
IDEA_PATHS_SELECTOR=CallMeMaybeMPS1.0

message()
{
  TITLE="Cannot start CallMeMaybeMPS"
...
# Try (in order): $CallMeMaybeMPS_JDK, .../callmemaybemps.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$CallMeMaybeMPS_JDK" ] && [ -x "$CallMeMaybeMPS_JDK/bin/java" ]; then
  JDK="$CallMeMaybeMPS_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/CallMeMaybeMPS1.0/callmemaybemps.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/CallMeMaybeMPS1.0/callmemaybemps.jdk")
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either CallMeMaybeMPS_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$CallMeMaybeMPS_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$CallMeMaybeMPS_PROPERTIES"
fi
...
if [ -n "$IDEA_VM_OPTIONS" ] && [ -r "$IDEA_VM_OPTIONS" ]; then
  ...
  VMOPTIONS_FNAME=callmemaybemps${BITS}.vmoptions
  # 
...
# shellcheck disable=SC2154
if [ -n "$CallMeMaybeMPS_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$CallMeMaybeMPS_CLASSPATH"
fi
...
"$JAVA_BIN" \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_CallMeMaybeMPS_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_CallMeMaybeMPS.hprof" \
