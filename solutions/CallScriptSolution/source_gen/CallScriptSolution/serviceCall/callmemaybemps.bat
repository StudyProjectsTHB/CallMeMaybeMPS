...
:: Generated by MPS
::----------------------------------------------------------------------
SET IDEA_PATHS_SELECTOR=CallMeMaybeMPS1.0
SET PRODUCT=callmemaybemps
...
IF "%IDE_BIN_DIR:~-8%" == "bin\win\" (
  echo.
  echo    You are trying to run CallMeMaybeMPS from generic distribution
  echo Please first copy content of "%IDE_BIN_DIR%"
  echo into its parent directory ("%IDE_BIN_DIR:~0,-4%"^)
  echo and then run callmemaybemps.bat from there
  echo.
  pause
  exit
)
...
:: Try (in order): CallMeMaybeMPS_JDK, callmemaybemps%BITS%.exe.jdk, ..\jre, JDK_HOME, JAVA_HOME.
:: ---------------------------------------------------------------------
SET JDK=
SET IDEA_VENDOR_NAME=null

IF EXIST "%CallMeMaybeMPS_JDK%" SET "JDK=%CallMeMaybeMPS_JDK%"
IF EXIST "%JDK%" GOTO check

SET BITS=64
SET USER_JDK64_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\CallMeMaybeMPS1.0\callmemaybemps%BITS%.exe.jdk
SET BITS=
SET USER_JDK_FILE=%APPDATA%\%IDEA_VENDOR_NAME%\CallMeMaybeMPS1.0\callmemaybemps%BITS%.exe.jdk
...
IF NOT EXIST "%JAVA_EXE%" (
  ECHO ERROR: cannot start CallMeMaybeMPS.
  ECHO No JDK found. Please validate either CallMeMaybeMPS_JDK, JDK_HOME or JAVA_HOME points to valid JDK installation.
  EXIT /B
)
...
:: ---------------------------------------------------------------------
IF NOT "%CallMeMaybeMPS_PROPERTIES%" == "" SET IDE_PROPERTIES_PROPERTY="-Didea.properties.file=%CallMeMaybeMPS_PROPERTIES%"

SET VM_OPTIONS_FILE=
SET USER_VM_OPTIONS_FILE=
IF NOT "%CallMeMaybeMPS_VM_OPTIONS%" == "" (
  :: 1. %<IDE_NAME>_VM_OPTIONS%
  IF EXIST "%CallMeMaybeMPS_VM_OPTIONS%" SET "VM_OPTIONS_FILE=%CallMeMaybeMPS_VM_OPTIONS%"
)
