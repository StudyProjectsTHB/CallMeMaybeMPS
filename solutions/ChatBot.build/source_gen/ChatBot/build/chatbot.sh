...
# ---------------------------------------------------------------------
# Generated by MPS
IDEA_PATHS_SELECTOR=ChatBot1.0

message()
{
  TITLE="Cannot start ChatBot"
...
# Try (in order): $ChatBot_JDK, .../chatbot.jdk, .../jbr, .../jbr-x86, $JDK_HOME, $JAVA_HOME, "java" in $PATH.
# ---------------------------------------------------------------------
IDEA_VENDOR_NAME='null'
# shellcheck disable=SC2154
if [ -n "$ChatBot_JDK" ] && [ -x "$ChatBot_JDK/bin/java" ]; then
  JDK="$ChatBot_JDK"
fi

if [ -z "$JDK" ] && [ -s "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/ChatBot1.0/chatbot.jdk" ]; then
  USER_JRE=$("$CAT" "${XDG_CONFIG_HOME:-$HOME/.config}/$IDEA_VENDOR_NAME/ChatBot1.0/chatbot.jdk")
...
if [ -z "$JDK" ] || [ ! -x "$JAVA_BIN" ]; then
  ...
  message "No JDK found. Please validate either ChatBot_JDK, JDK_HOME or JAVA_HOME environment variable points to valid JDK installation."
  fi
  exit 1
fi
...
# ---------------------------------------------------------------------
# shellcheck disable=SC2154
if [ -n "$ChatBot_PROPERTIES" ]; then
  IDE_PROPERTIES_PROPERTY="-Didea.properties.file=$ChatBot_PROPERTIES"
fi
...
if [ -n "$IDEA_VM_OPTIONS" ] && [ -r "$IDEA_VM_OPTIONS" ]; then
  ...
  VMOPTIONS_FNAME=chatbot${BITS}.vmoptions
  # 
...
# shellcheck disable=SC2154
if [ -n "$ChatBot_CLASSPATH" ]; then
  CLASSPATH="$CLASSPATH:$ChatBot_CLASSPATH"
fi
...
"$JAVA_BIN" \
  -classpath "$CLASSPATH" \
  ${VM_OPTIONS} \
  "-XX:ErrorFile=$HOME/java_error_in_ChatBot_%p.log" \
  "-XX:HeapDumpPath=$HOME/java_error_in_ChatBot.hprof" \
